...
class ThreadAccessibilityService : AccessibilityService() {
    ...
    private fun onWindowChanged(pkg: String, e: AccessibilityEvent, now: Long) {
        val tracked = observedPackage ?: return
    ...
    fun evaluateAfterReturn(now: Long) {
        ...
        val sml = Sml.lookup(state.currentScreenId, complexityCache)
    ...
    /**
     * A task begins when an app says one has begun.
     *
     * Inferring task boundaries from the node tree alone is possible but
     * unreliable, and the cost of getting it wrong is asymmetric: a task that
     * starts late loses the very decisions worth restoring, and one that never
     * ends holds context it has no business holding. So Tier 2 states it, and
     * Tier 1 without an integrated app simply holds nothing.
     */
